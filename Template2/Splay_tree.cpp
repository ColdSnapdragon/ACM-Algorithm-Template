...
int main()
{
    ...
    for(int i=1;i<=Q;++i)
    {
        long long p,x;
        cin>>p>>x;
        if(p==1) T.insert((x<<32)+i); //+i是为了去重，“<<32”使得int的负数在映射为long long后还是负数
        else if(p==2) T.erase(T.lower_bound(x<<32));
        else if(p==3) cout<<T.order_of_key(x<<32)+1<<"\n";
        else
        {
            long long ans;
            if(p==4) ans=*T.find_by_order(x-1);
            if(p==5) ans=*--T.lower_bound(x<<32);
            if(p==6) ans=*T.lower_bound((x+1)<<32);
            ans>>=32;
            cout<<ans<<"\n";
        }
    }
